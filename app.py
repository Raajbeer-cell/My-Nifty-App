charts.", "sector_rank": "3" }, { "script_name": "INFOSYS", "trend_strength": "Weak/Bearish (ADX: 18)", "entry_price": 1820, "stop_loss": 1880, "exit_price_target_1": 1750, "exit_price_target_2": 1680, "holding_duration": "7-10 Days", "expected_returns": "5-7% (Short Side)", "logic": "Facing resistance at 1850; Tech sector under pressure due to global discretionary spending slowdown.", "sector_rank": "Lagging" } ], "METALS": [ { "commodity": "GOLD (MCX)", "trend": "BULLISH", "current_market_price": 148676, "entry_range": "148000 - 148500", "stop_loss": 146500, "target_1": 152000, "target_2": 155000, "analysis": "Safe-haven buying amid global volatility. Price sustaining above 20-day EMA." }, { "commodity": "SILVER (MCX)", "trend": "HIGH MOMENTUM BULLISH", "current_market_price": 254005, "entry_range": "252000 - 253000", "stop_loss": 248000, "target_1": 265000, "target_2": 275000, "analysis": "Industrial demand uptick and breakout from consolidation zone. Silver outperforming Gold in percentage terms." }, { "commodity": "COPPER", "trend": "NEUTRAL", "current_market_price": 865, "entry_range": "Wait for dip to 850", "stop_loss": 835, "target_1": 890, "target_2": 910, "analysis": "Consolidating in a narrow range. Wait for breakout above 880 for fresh longs." } ], "SCRIPT_SPECIFIC_SIGNALS": [ { "script_name": "NIFTY 50", "side": "BUY/LONG", "action_zone": "Above 25800", "confirmation_logic": "Hourly closing above 25800 indicates continuation of the trend towards 26000. Support established at 25650.", "risk_reward": "1:2.5" }, { "script_name": "BANK NIFTY", "side": "WAIT/WATCH", "action_zone": "Range 59800 - 60300", "confirmation_logic": "Index is stuck in a consolidation box. Buy only on a clear breakout above 60350 with high volumes.", "risk_reward": "1:2" }, { "script_name": "BITCOIN", "side": "BUY/LONG", "action_zone": "Above $110,000", "confirmation_logic": "Holding above key support. Altseason delayed—BTC dominance still high.", "risk_reward": "1:3" }, { "script_name": "CRUDE OIL", "side": "SELL/SHORT", "action_zone": "Below $72", "confirmation_logic": "Bearish bias intact with oversupply concerns. Target $68.", "risk_reward": "1:2" } ], "GLOBAL_NEWS_IMPACT": [ { "event": "India-US Trade Deal Finalized", "sentiment": "BULLISH", "impact_asset": "Indian Equities (Pharma, IT Services, Exports)", "reason": "Reduces tariff barriers. Long-term positive." }, { "event": "Fed Hints at Rate Cuts in Q2 2026", "sentiment": "BULLISH", "impact_asset": "Gold, Silver, Global Equities", "reason": "Lower rates = weaker USD = boost for commodities and risk-on assets." }, { "event": "China GDP Miss (Expected 5.2%, Actual 4.8%)", "sentiment": "BEARISH", "impact_asset": "Base Metals (Copper), Energy", "reason": "Slowing demand from world's largest commodity consumer." } ] }

--- FETCH DATA ---
@st.cache_data(ttl=60, show_spinner=False) def fetch_data(tickers, period, interval): try: data = yf.download(tickers, period=period, interval=interval, group_by='ticker', progress=False) return data except Exception as e: st.error(f"Data fetch error: {e}") return None

--- TECHNICAL SCORING ---
def calculate_technical_score(df): df_ind = df.copy()
...
return score, positive_signals, total_checks, checks--- MAIN APP UI ---
Updated to 3 Tabs to include the new Data
tab1, tab2, tab3 = st.tabs(["📊 DASHBOARD", "📰 INTELLIGENCE", "🎯 PRIME SETUPS"])

--- TAB 1: LIVE DASHBOARD ---
with tab1: col_ctrl, col_status = st.columns([4, 1]) with col_ctrl: timeframe = st.selectbox("Market View:", ["15m", "1h", "1d"], index=1)
...
with tab2: st.header("🌍 Market Sentiment News") news_opt = st.radio("Source:", ["General", "India", "Crypto"], horizontal=True)
...
with tab3: st.markdown("### 🏆 Expert Swing Picks & Analysis")
